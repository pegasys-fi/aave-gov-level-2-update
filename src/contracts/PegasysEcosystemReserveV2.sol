...
interface IStreamable {
  ...
  function initialize(uint256 proposalId, address pegasysGovernanceV2) external;
}
...
/**
 * @title PegasysEcosystemReserve v2
 ...
 * Modifications:
 * - Sablier "pulls" the funds from the creator of the stream at creation. In the Pegasys case, we already have the funds.
 * - Anybody can create streams on Sablier. Here, only the funds admin (Pegasys governance via controller) can
 ...
 * @author BGD Labs
 **/
contract PegasysEcosystemReserveV2 is
  ...
  IStreamable
{
   ...
   * @param pegasysGovernanceV2 address of the Pegasys governance
   */
  function initialize(
    uint256 proposalId,
    address pegasysGovernanceV2
  ) external initializer {
    // voting process
    IPegasysGovernanceV2 pegasysGov = IPegasysGovernanceV2(pegasysGovernanceV2);
    pegasysGov.submitVote(proposalId, true);
  }
